...
class MessageRepository:
    ...
    def update_status(self, message_id: str, *, status: str, sent_at: str | None = None, error_message: str | None = None, provider_message_id: str | None = None) -> None:
        with get_connection() as conn:
            conn.execute(
                """
                UPDATE messages
    ...
    def get_tracked_link(self, token: str) -> dict[str, Any] | None:
        with get_connection() as conn:
            row = conn.execute(
                "SELECT * FROM tracked_links WHERE token = ?",
                (token,),
            ).fetchone()
